...
import { FolderOpen, X, ChevronDown, Command, Search, RotateCcw, ArrowLeft, ArrowRight, Layout, Settings, HelpCircle, Moon, Sun } from 'lucide-react';

const TitleBar = ({ onOpenFolder, activeFile, projectName }) => {
  const [isDarkBlack, setIsDarkBlack] = React.useState(true);

  const toggleTheme = () => {
    const root = document.documentElement;
    if (isDarkBlack) {
      // Switch back to original deep charcoal theme
      root.style.setProperty('--color-background', '#0d1117');
      root.style.setProperty('--color-sidebar', '#161b22');
      root.style.setProperty('--color-activity', '#010409');
      root.style.setProperty('--color-border', '#30363d');
      root.style.setProperty('--color-primary', '#58a6ff');
      root.style.setProperty('--color-secondary', '#21262d');
      root.style.setProperty('--color-text-main', '#c9d1d9');
      root.style.setProperty('--color-text-dim', '#8b949e');
    } else {
      // Switch to pure black theme
      root.style.setProperty('--color-background', '#000000');
      root.style.setProperty('--color-sidebar', '#0a0a0a');
      root.style.setProperty('--color-activity', '#050505');
      root.style.setProperty('--color-border', '#1a1a1a');
      root.style.setProperty('--color-primary', '#ffffff');
      root.style.setProperty('--color-secondary', '#121212');
      root.style.setProperty('--color-text-main', '#e0e0e0');
      root.style.setProperty('--color-text-dim', '#707070');
    }
    setIsDarkBlack(!isDarkBlack);
  };
    ...
    <header className="h-9 bg-activity border-b border-border flex items-center px-3 select-none justify-between overflow-hidden">
      ...
      <div className="flex items-center space-x-1">
        ...
        <div className="flex items-center space-x-0.5">
          <button 
            onClick={toggleTheme} 
            className="p-1.5 text-text-dim hover:text-text-main rounded-md transition-all flex items-center"
            title={isDarkBlack ? "Switch to Deep Charcoal" : "Switch to Pure Black"}
          >
            {isDarkBlack ? <Sun size={14} /> : <Moon size={14} />}
          </button>
